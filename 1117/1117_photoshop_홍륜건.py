...
window,window2,window3,window4,window5,window6,window7,window8,window9 = None, None, None, None, None,None, None, None, None
canvas2,canvas3,canvas4,canvas5,canvas6,canvas7,canvas8,canvas9 = None, None, None, None,None, None, None, None
paper2,paper3,paper4,paper5,paper6,paper7,paper8,paper9 = None, None, None, None,None, None, None, None
readFp = None
...
def displayImage(img, width, height): #이미지를 화면에 출력하는 함수
    global window,window2,window3,window4,window5,window6,window7,window8,window9, canvas2,canvas3,canvas4,canvas5,canvas6,canvas7,canvas8,canvas9, paper, photo, photo2, oriX, oriY
    ...
    w = 0
    c = 0
    win = [window2,window3,window4,window5,window6,window7,window8,window9]
    can = [canvas2,canvas3,canvas4,canvas5,canvas6,canvas7,canvas8,canvas9]
    pap = [paper2,paper3,paper4,paper5,paper6,paper7,paper8,paper9]
    for i in range(len(win)):
        if win[i]== None:
            win[i] =Toplevel()
            win[i].wm_attributes("-topmost", 1)
            win[i].title(str(n[s-1]))
            win[i].geometry(str(width)+"x"+str(height)+"+500+500")
            w = win[i]
            can[i] =Canvas(w, width = width , height=height, bg ='#626262', bd= 0, highlightthickness = 0)
            c = can[i]
            break
        else:
            continue
    ...
    #window2.title(str(p[s-1]))
    #window2.geometry(str(width)+"x"+str(height)+"+500+500")

    #새 캔버스 생성, 처리된 이미지의 가로 세로 사이즈대로 생성
  
        
    #canvas = Canvas(w, width = width , height=height, bg ='#626262', bd= 0, highlightthickness = 0)
    ...
    paper.put(blob)
    c.create_image((width/2, height/2), image=paper, state ="normal")
